...
namespace eAppointment.Backend.Application.Features.Cities.GetAllCountiesByCityId
{
    public sealed class GetAllCountiesByCityIdQueryHandler(
        ICountyRepository countyRepository) : IRequestHandler<GetAllCountiesByCityIdQuery, Result<List<GetAllCountiesByCityIdQueryResponse>>>
    {
        public async Task<Result<List<GetAllCountiesByCityIdQueryResponse>>> Handle(GetAllCountiesByCityIdQuery request, CancellationToken cancellationToken)
        {
            ...
            List<GetAllCountiesByCityIdQueryResponse> response =
                counties.Select(s =>
                    new GetAllCountiesByCityIdQueryResponse
                    (
                        s.Id,
                        s.Name
                    )).ToList();
